...
const SystemDiscipline = ({
  ...
  const handleDelete = async (id: string) => {
    ...
    // const apiUrl = `http://192.168.1.205:3001/admin/discipline/${id}`;
    const apiUrl = `${getApiUrl("/admin/discipline")}/${id}`;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || "Erreur lors de la suppression de la Discipline"
        );
      }
  ...
  const handleUpdate = async (id: string, newName: string) => {
    ...
    if (!newName) {
      toast.error("Veuillez renseigner le nom de la discipline.");
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.message || "Erreur lors de la mise à jour de la discipline"
        );
      }
            ...
            <div>
              ...
              <div className="flex flex-row justify-between py-2">
                ...
                <div className="flex flex-row gap-8">
                  <button onClick={() => openEditModal("Administrations")}>
                    <Image
...
export default SystemDiscipline;
